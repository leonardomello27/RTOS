...
TASK(Can_Receive_Simu_arduino)
{
	if(!digitalRead(2)){  
		...
		if((mID & ID_Ego_speed) == ID_Ego_speed) {
			...
			//Serial.print("ENTROU NA TASK EGO_SPEED = ");
			//Serial.print(Ego_speed);
			Serial.println("");
			Serial.println("RECEBEU speed");
			Serial.print("ID = ");
			Serial.println(mID);
		...
		if((mID & ID_Relative_distance) == ID_Relative_distance) {
			Relative_distance = (mDATA[1] << 8) | mDATA[2]; //Check data dictionary
			ReleaseResource(res1);
			TerminateTask();
		}
		if((mID & ID_Relative_speed) == ID_Relative_speed) {
			Relative_speed = (mDATA[1] << 8) | mDATA[2]; //Check data dictionary
