 ...
 * Automatically generated by goil on Thu Sep 14 20:30:08 2023
...
CONST(tpl_appmode_mask, OS_CONST) tpl_alarm_app_mode[ALARM_COUNT] = {
  ...
  1 /* alarm periodicAlarmSendCAN : stdAppmode */ 
};
...
/* Task sendRelativeSpeed identifier */
#define sendRelativeSpeed_id 0
CONST(TaskType, AUTOMATIC) sendRelativeSpeed = sendRelativeSpeed_id;

/* Task Receive identifier */
#define Receive_id 1
CONST(TaskType, AUTOMATIC) Receive = Receive_id;

/* Task sendDistance identifier */
#define sendDistance_id 2
CONST(TaskType, AUTOMATIC) sendDistance = sendDistance_id;

/* Task Environment identifier */
#define Environment_id 3
CONST(TaskType, AUTOMATIC) Environment = Environment_id;

/* Task sendSpeed identifier */
#define sendSpeed_id 4
CONST(TaskType, AUTOMATIC) sendSpeed = sendSpeed_id;
...
/* Alarm periodicAlarmSendCAN identifier */
#define periodicAlarmSendCAN_id 2
CONST(AlarmType, AUTOMATIC) periodicAlarmSendCAN = periodicAlarmSendCAN_id;
 ...
 */
/*-----------------------------------------------------------------------------
 * Task sendRelativeSpeed descriptor
 */
#define APP_Task_sendRelativeSpeed_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task sendRelativeSpeed function prototype
 */

FUNC(void, OS_APPL_CODE) sendRelativeSpeed_function(void);
#define APP_Task_sendRelativeSpeed_STOP_SEC_CODE
...
/*
 * Task sendRelativeSpeed stack
 *
 */
#define APP_Task_sendRelativeSpeed_START_SEC_STACK
#include "tpl_memmap.h"
tpl_stack_word sendRelativeSpeed_stack_zone[128/sizeof(tpl_stack_word)];
#define APP_Task_sendRelativeSpeed_STOP_SEC_STACK
#include "tpl_memmap.h"

#define OS_START_SEC_VAR_UNSPECIFIED
#include "tpl_memmap.h"

#define sendRelativeSpeed_STACK { sendRelativeSpeed_stack_zone, 128 }

/*
 * Task sendRelativeSpeed context
 */
avr_context sendRelativeSpeed_int_context;
#define sendRelativeSpeed_CONTEXT &sendRelativeSpeed_int_context
...
/*
 * Static descriptor of task sendRelativeSpeed
 */
CONST(tpl_proc_static, OS_CONST) sendRelativeSpeed_task_stat_desc = {
  /* context                  */  sendRelativeSpeed_CONTEXT,
  /* stack                    */  sendRelativeSpeed_STACK,
  /* entry point (function)   */  sendRelativeSpeed_function,
  /* internal ressource       */  NULL,
  /* task id                  */  sendRelativeSpeed_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task sendRelativeSpeed
 */
VAR(tpl_proc, OS_VAR) sendRelativeSpeed_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
...
#include "tpl_memmap.h"
tpl_stack_word Receive_stack_zone[256/sizeof(tpl_stack_word)];
...
#define Receive_STACK { Receive_stack_zone, 256 }
...
/*-----------------------------------------------------------------------------
 * Task sendDistance descriptor
 */
#define APP_Task_sendDistance_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task sendDistance function prototype
 */

FUNC(void, OS_APPL_CODE) sendDistance_function(void);
#define APP_Task_sendDistance_STOP_SEC_CODE
...
/*
 * Task sendDistance stack
 *
 */
#define APP_Task_sendDistance_START_SEC_STACK
#include "tpl_memmap.h"
tpl_stack_word sendDistance_stack_zone[128/sizeof(tpl_stack_word)];
#define APP_Task_sendDistance_STOP_SEC_STACK
#include "tpl_memmap.h"

#define OS_START_SEC_VAR_UNSPECIFIED
#include "tpl_memmap.h"

#define sendDistance_STACK { sendDistance_stack_zone, 128 }

/*
 * Task sendDistance context
 */
avr_context sendDistance_int_context;
#define sendDistance_CONTEXT &sendDistance_int_context
...
/*
 * Static descriptor of task sendDistance
 */
CONST(tpl_proc_static, OS_CONST) sendDistance_task_stat_desc = {
  /* context                  */  sendDistance_CONTEXT,
  /* stack                    */  sendDistance_STACK,
  /* entry point (function)   */  sendDistance_function,
  /* internal ressource       */  NULL,
  /* task id                  */  sendDistance_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task sendDistance
 */
VAR(tpl_proc, OS_VAR) sendDistance_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
...
#include "tpl_memmap.h"
tpl_stack_word Environment_stack_zone[256/sizeof(tpl_stack_word)];
...
#define Environment_STACK { Environment_stack_zone, 256 }
...
/*-----------------------------------------------------------------------------
 * Task sendSpeed descriptor
 */
#define APP_Task_sendSpeed_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task sendSpeed function prototype
 */

FUNC(void, OS_APPL_CODE) sendSpeed_function(void);
#define APP_Task_sendSpeed_STOP_SEC_CODE
...
/*
 * Task sendSpeed stack
 *
 */
#define APP_Task_sendSpeed_START_SEC_STACK
#include "tpl_memmap.h"
tpl_stack_word sendSpeed_stack_zone[128/sizeof(tpl_stack_word)];
#define APP_Task_sendSpeed_STOP_SEC_STACK
#include "tpl_memmap.h"

#define OS_START_SEC_VAR_UNSPECIFIED
#include "tpl_memmap.h"

#define sendSpeed_STACK { sendSpeed_stack_zone, 128 }

/*
 * Task sendSpeed context
 */
avr_context sendSpeed_int_context;
#define sendSpeed_CONTEXT &sendSpeed_int_context
...
/*
 * Static descriptor of task sendSpeed
 */
CONST(tpl_proc_static, OS_CONST) sendSpeed_task_stat_desc = {
  /* context                  */  sendSpeed_CONTEXT,
  /* stack                    */  sendSpeed_STACK,
  /* entry point (function)   */  sendSpeed_function,
  /* internal ressource       */  NULL,
  /* task id                  */  sendSpeed_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task sendSpeed
 */
VAR(tpl_proc, OS_VAR) sendSpeed_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
...
/*-----------------------------------------------------------------------------
 * Alarm periodicAlarmSendCAN descriptor
 *
 * This alarm does the activation of task sendRelativeSpeed
 */
...
CONST(tpl_task_activation_action, OS_CONST) periodicAlarmSendCAN_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  sendRelativeSpeed_id
...
CONST(tpl_alarm_static, OS_CONST) periodicAlarmSendCAN_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_ID == YES)
    /* id of the alarm for tracing  */  , periodicAlarmSendCAN_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&periodicAlarmSendCAN_action
};
...
VAR(tpl_time_obj, OS_VAR) periodicAlarmSendCAN_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&periodicAlarmSendCAN_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  30,
    /* date                         */  3000,
...
CONSTP2VAR(tpl_time_obj, AUTOMATIC, OS_APPL_DATA)
  ...
  &periodicAlarmSendCAN_alarm_desc
};
#define OS_STOP_SEC_CONST_UNSPECIFIED
#include "tpl_memmap.h"
