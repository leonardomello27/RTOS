...
#define ACC_Status_ID 		0x18FEF100  //Receives data regarding the ACC status, such as enabled/disabled, faulty sensors, etc.
#define Simulation_Data_ID	0x0C1B3049  //Receives data from simulation such as car current speed.
...
static 				byte M  = 0; //Used to store result of sending CAN messages.
...
TASK(Receive)
{
	if(!digitalRead(2)){
		...
		if((mID & ACC_Status_ID) == ACC_Status_ID){
			ACC_enabled = mDATA[1];
